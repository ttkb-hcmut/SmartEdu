...
def _to_items(video_id: str, whisper_segments: List[Dict]) -> List[Dict]:
    ## map whisper segments to group_passages items — valley-cut on time axis
    return [{
        "id": f"{video_id}_s{i}",
        "text": seg["text"],
        "order": i,
        "section_id": video_id,            # one video = one section, no headings
...
def build_video_segments(embedder, milvus_db, video_id: str, whisper_segments: List[Dict],
    ...
    ## returns (segment_nodes, anchor_links, novel_entries) — pure, no db writes
    ...
    for order, p in enumerate(merged):
        ...
        ## anchor only kept hits that also clear the absolute floor
        for i in kept:
